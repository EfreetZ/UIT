...
bool GObject::Achive()  //判断物体是否到达逻辑坐标位置
{
    ...
    bool i = (n == 0 && k == 0);         // 如果两个余数都为0,说明到达中心位置
    return i;
}
...
{
    // 申请资源,并交给智能指针处理
    HBRUSH hbr = ::CreateSolidBrush(RGB(255, 255, 255));//创建画刷，绘制矩形函数使用
...
bool GObject::Collision()   // 碰撞检测
{
    bool b = false;         //没撞墙

    //更新行、列的数据若是大嘴,则会执行PacMan重写的AchiveCtrl函数消除豆子
    AchiveCtrl();   //到达逻辑点后更新数据
    //判断指令的有效性
    if (m_nArray < 0 || m_nRow < 0 || m_nArray > MAPLENTH - 1|| m_nRow > MAPLENTH - 1)  //大于零 小于边界
    {
        b = true;
    }
    else if (Achive()) 
    {
        ...
        case LEFT:
            //判断下一个格子是否能够通行
            if (m_nArray > 0 && !pStage->mapData[m_nRow][m_nArray - 1]) // mapData 障碍物逻辑地图点阵
            {
                b = true;//指令无效，撞墙了
            }
            break;
...
void PacMan::Draw(HDC& memDC)
{
    ...
    else if (m_nFrame % 2 == 0) {   //第4祯动画与第2祯动画    本体张嘴形状
...
void RedOne::MakeDecision(bool b)
{
    ...
    if (i % 4 == 0) {
        ...
        m_dir != DOWN ? m_cmd = UP : m_cmd = DOWN;      //非向下移动则使之面向下，否则面向下
    }
    else if (i % 2 == 0) {
        m_dir != RIGHT ? m_cmd = LEFT : m_cmd = RIGHT;  //非向右移动则使之面向下，否则面向右
    }
    else {
        m_dir != LEFT ? m_cmd = RIGHT : m_cmd = LEFT;   //非向左移动则使之面向下，否则面向左
    }
}
//BlueOne成员定义
